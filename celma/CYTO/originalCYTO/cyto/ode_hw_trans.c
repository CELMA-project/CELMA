...
/* Minimum density  */
#define NMIN 1.e-16
#define MINERROR 1.e-14 /* Minimal error */
#define MINOM 1.e-6/* Accuracy Omega minimal */
#define MAXOM 1.e-7 /* Do not refine range below */
#define ARGON 40.
    ...
    for(par=par_start;dpar*par <= dpar*par_end;par+=dpar)
    {
        ...
        for(im=1;im<=LASTM;im++)
        {
            ...
            {   
                ...
                FOUND = FALSE;
                omega[im-1][2] = FALSE;
                fprintf(stderr, "Search interval [%f,%f]x[%f,%f]\n",omrs,omre,omis,omie);
                    ...
                    for(omi=omie;omi>=omis;omi-=domi) /* search for mode with largest growthrate */
                    {
                        ...
                        */
                        ...
                        for(i=0;i<3;i++)
                        {
                            ...
                            ys[i][0] = 1./tpm/t*y[i][2]-m/t*y[i][0];
                            ys[i][1] = 1./tpm/t*y[i][3]-m/t*y[i][1];

                            ys[i][2] = (m*m/t* y[i][0] - Qr*y[i][0]+Qi*y[i][1])*tpm + kappar*y[i][2] - kappai*y[i][3] ;
                            ys[i][3] = (m*m/t* y[i][1] - Qr*y[i][1]-Qi*y[i][0])*tpm + kappar*y[i][3] + kappai*y[i][2];
                        ...
                        for(i=0;i<N-1;i++) 
                        {
                            ...
                            y1r = pow(t,(int)m)*y[i][0];
                            y1i = pow(t,(int)m)*y[i][1];
                               if(t>0.7*B)
                                if( (2.*dx_density[i]*density[i]*(y1r*y1r+y1i*y1i)
                                                 +density[i]*density[i]*2./rcor[i]*(y1r*y[i][2]+y1i*y[i][3])) > 0. ) 
                                {
                                    status = !GSL_SUCCESS;
                         ...
                         /* Solution should be growing in amplitude at innermostmost 5 % of domain */
                               if(t<0.05*B)
                                if( (2.*dx_density[i]*density[i]*(y1r*y1r+y1i*y1i)
                                                 +density[i]*density[i]*2./rcor[i]*(y1r*y[i][2]+y1i*y[i][3])) < 0. ) status = !GSL_SUCCESS;
                        ...
                        else
                        {
                            ...
                            in = 0;
                            for(i=10;i<N-10;i++)if(y[i][0]*y[i+1][0] < 0.0 ) in++;
                                ...
                                for(i=0;i<N;i++)  
                                    {
                                        ...
                                        y1r = pow(t,(int)(m))*y[i][0];
                                        y1i = pow(t,(int)(m))*y[i][1];            
                                        ys[i][0] = density[i]*density[i]*(y1r*y1r+y1i*y1i);
                                    }
                                
                                norm = 0.;
                                for(i=0;i<N;i++) norm = MAX(norm ,ys[i][0]);
                                error = sqrt(ys[N-5][0]/norm);
                            }
                            else
                            {
                                error = 99.;
      ...
      void *params)
{
    ...
    f[0]=  edtpm*y[2]*edt-m*edt*y[0];
    f[1] = edtpm*y[3]*edt-m*edt*y[1];
    
    f[2] = (y[0]*m2edt -Qr*y[0] + Qi*y[1])/edtpm  + (kappar*y[2] - kappai*y[3]);
    f[3] = (y[1]*m2edt  -Qr*y[1] - Qi*y[0])/edtpm  + (kappar*y[3] + kappai*y[2]);
     ...
     double dfdt[], void *params)
{
    ...
    dfdt[0] = (m*y[0]-edtpm*(m+1.) *y[2])/(t*t);
    dfdt[1] = (m*y[1]-edtpm*(m+1.) *y[3])/(t*t);

   dfdt[2] = -m*m/t/t *y[0]
        +(krp-kappar)/h*y[2]-(kip-kappai)/h*y[3]
        -(Qrp-Qr)/h*y[0]+(Qip-Qi)/h*y[1]
               +m/t/edtpm*(m*m/t*y[0]  -Qr*y[0]+Qi*y[1]);
    dfdt[3] = -m*m/t/t *y[1]
        +(krp-kappar)/h*y[3]+(kip-kappai)/h*y[2]
        -(Qrp-Qr)/h*y[1]-(Qip-Qi)/h*y[0]
               +m/t/edtpm*(m*m/t*y[1]  -Qr*y[1]-Qi*y[0]);;
    return GSL_SUCCESS;
}
