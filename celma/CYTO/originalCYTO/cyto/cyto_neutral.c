...
int main(int argc,char **argv)
{
    ...
    // Setting these wll write the corresponding snapshot files 
    int 
      W_UION   = TRUE,
      W_DZN    = FALSE,
      W_H0     = FALSE,
    ...
    HDF_DS 
        data = {{0}}, 
        *d  = NULL; // compiler will warn about missing braces, but this is c99 standard
    PARA   
        para = {0}, 
        *p = NULL;
...
// Neutrals Fast species
	double 
        ...
        **H_fast_bdz[2]   = {NULL,NULL};

	// neutrals other fields
	double 
        ***G_ion    = NULL,
        ***G_rec    = NULL,
        ***G_cx     = NULL,
		***Momentum_source = NULL;
    ...
    */
    ...
    p->otmult               = 10; //Number of small outputs  before output of fields
  ...
  // fast neutrals
	...
	// Neutral interaction fields

	G_ion          = Util_DCube(nx,d->offx,ny,d->offy,nz,d->offz,0); // ionisation  
	G_cx           = Util_DCube(nx,d->offx,ny,d->offy,nz,d->offz,0); // charge exchange  
	G_rec          = Util_DCube(nx,d->offx,ny,d->offy,nz,d->offz,0); // recombination    
	Momentum_source =  Util_DCube(nx,d->offx,ny,d->offy,nz,d->offz,0);     
    ...
    Cyto_Geometry(ispolar,rcor,edrcor,zhval,zgval,zcor,d, p);
    ...
    /****************************************************/
    /*                                                  */
    /* Restart and start capabilities                             */
    /*                                                  */
    /****************************************************/
    ...
    Cyto_Start_Restart(n_0,w_0,U_0,V_0,t_0,zcor,d,p);
    
    // Initialise omega and f_0 

    FORALL_BD omega[iz][ip][ir] = 0.;
    FORALL_BD f_0[iz][ip][ir] = 0.;
    

  // SET  density source :Localisation of density source in first quarter of device
   ...
   // SET  shape function to drive vorticity to zero in the vicinity of the radial wall
    ...
    // SET neutral density to be 1 to 0.5

    FORALL_BD   
        H_slow_0[iz][ip][ir] = log((0.5+0.5*(1.-zcor[iz]/Lz))*p->n_n/p->n0);



    //fprintf(stderr,"Ratio neutral/plasma density: %f\n",p->n_n/p->n0);
   ...
   /****************************************************/
    ...
    if(ISROOT) fprintf(stderr," Unit                              : %f [m]\n",p->unit_length);
    if(ISROOT) fprintf(stderr,"\n");  
    ...
    if(ISROOT) fprintf(stderr," Unit                              : %f [us]\n",p->unit_time);
   BUGREPORT;
   ...
   Neutral_plasma_interaction(d,p,G_ion,G_cx,G_rec,Momentum_source, H_slow_0, exp_n_0, exp_t_0);
 ...
 while(p->time < p->end_time )
  {
      if(FALSE == first )
      {
          ...
          // Parallel advection with electron speed as we look at electron density
          FORALL dtn_0[iz][ip][ir]   +=  -dzVN[iz][ip][ir];
          
          // density source from neutrals, rates alread normalised by n!
          //FORALL    dtn_0[iz][ip][ir] +=  (G_ion[iz][ip][ir]-G_rec[iz][ip][ir]);
          ...
          // Calculate dt ln n_0 .....with implicit term
          ...
          // density source, compensate w should be equivalent to pedersen current
		  // FORALL    dtw_0[iz][ip][ir] += -(G_rec[iz][ip][ir]+ G_cx[iz][ip][ir])*omega[iz][ip][ir]*0.;
          ...
          // parallel gradient 
          FORALL  dtt_0[iz][ip][ir]  += -(V_0[iz][ip][ir]+p->alpha)*dzt[iz][ip][ir] - 0.0*tf*dzV[iz][ip][ir];
    ...
    COMM(fprintf(stderr,"t = %f, before static  n = %g  f = %g W = %g om = %g \n",p->time,  
                   exp_n_0[nz/2][ny/2][nx/2], f_0[nz/2][ny/2][nx/2], w_0[nz/2][ny/2][nx/2] ,omega[nz/2][ny/2][nx/2] ););
      ...
      FORZY_BD  nbdr[1][iz][ip]   = 0.5*(3.*n_0[iz][ip][nx-1]-n_0[iz][ip][nx-2]);  //n_0[iz][ip][nx-1]-1.;
      ...
      */
      ...
      // Solve of nonlinear Laplace equation
      // Initially f_0 = 0 and omega = 0
      for(i=0;i<3;i++)
      {
          FORALL  omega[iz][ip][ir] = w_0[iz][ip][ir]- dxn[iz][ip][ir]*dxf[iz][ip][ir]-dyn[iz][ip][ir]*dyf[iz][ip][ir];
          // calculate the circulation, integrate the vorticity omega in each slice....
          FORZ 
          {
              Util_Integral(omega[iz],1,norm[0],ny,nx,&ombdr[1][iz][ny]);
              ombdr[1][iz][-1]= ombdr[1][iz][ny]/= 2.*M_PI*p->xmax; //calc average from integral
              COMM(fprintf(stderr,"Circulation for iz = %d is %g\n", iz, ombdr[1][iz][-1]););
              
              
              FORY ombdr[1][iz][ip] = ombdr[1][iz][ny];
      ...
      FORALL  nu_vmu[iz][ip][ir]     =  nu_density[iz][ip][ir] * (V_0[iz][ip][ir]+ p->alpha -U_0[iz][ip][ir]);
...
;
      ...
      COMM(fprintf(stderr,"t = %f, wt = %f, ot = %d  n = %g  f = %g W = %g om = %g \n",p->time,write_time,ot,  
                   exp_n_0[nz/2][ny/2][nx/2], f_0[nz/2][ny/2][nx/2], w_0[nz/2][ny/2][nx/2] ,omega[nz/2][ny/2][nx/2] ););
      BUGREPORT;
      ...
      if(p->time>= write_time-0.5*p->dt)
      {
          ...
          */
          ...
          reducing[8] = PH_3DIntegral_noreduce(tfeld,1,norm,nz,ny,nx);
		   ...
		   for(iz=0;iz>nz;iz++) 
               for(ip=0;ip>ny;ip++) 
              reducing[9] = p->mue_n * dxn[iz][ip][nx-1];
                                           

          BUGREPORT;
          MPI_Allreduce((void *)&reducing[0], (void *) &result_vector[0], 10, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
         ...
         /**************************************************************/
          if((ot >= p->otmult) /*&& p->time < 2000*/)
          {
              ...
              if(W_DZVN) PH_3D_Write(dzVN,"dzVN",d->name_out,d->number,d,p,FALSE);
              if(W_DZVN) PH_3D_Write(VN,"VN",d->name_out,d->number,d,p,FALSE);

              PH_3D_Write(H_slow_0,"H-neutral",d->name_out,d->number,d,p,FALSE);
              PH_3D_Write(G_ion,"G_ion",d->name_out,d->number,d,p,FALSE);
              PH_3D_Write(G_rec,"G_rec",d->name_out,d->number,d,p,FALSE);
              PH_3D_Write(G_cx,"G_cx",d->name_out,d->number,d,p,FALSE);
  ...
  */
    ...
    if(offz == 1)   
    {
        if(data->grid_coords[0] == 0)
        {
            if(para->zmin < 0.0)
            {
              // a sheath condition for zmin  negative only, zmin positive means that we do not consider a sheath, but a mirror like situation
                BUGREPORT;
                switch(identity){
                    case IS_CURRENT:
                        FORYX_BD 
                    ...
                    case  IS_IONVELOCITY:
                        ...
                        FORYX_BD
                        {
                            bdval=-sqrt(exp_t[0][ip][ir]); // Note that velocity is in the negative z direction at the lower wall
                            val[-1][ip][ir] =2.*bdval-val[0][ip][ir];
                        }
                        break;
                    case IS_DENSITY:
            ...
            switch(identity){
                ...
                case  IS_ELECTRONVELOCITY:
                    ...
                    FORYX_BD  
                    {
                        ...
                        bdval =  sqrt(exp_t[nz-1][ip][ir])*exp(-val[nz][ip][ir]/exp_t[nz-1][ip][ir]); 
                ...
                case IS_DENSITY:
                    ...
                    {  
                        ...
                        // dz n = +/- n/c_s dz V_I which translates to 
                        // dz ln n = +/- dz V_i
                        bdval           =    -dzv[nz-1][ip][ir];
                        val[nz][ip][ir] = val[nz-1][ip][ir] +bdval/(0.5*zhval[nz-1]);
                    }
                    break;
                case IS_POTENTIAL: 
                    // No gradient in potential, sheeth works via parallel current
                    ...
                    // continous gradient, average dz v
                    FORYX_BD   val[nz][ip][ir] = -0.1*(4.*dzv[nz-1][ip][ir]+3.*dzv[nz-2][ip][ir]+2.*dzv[nz-3][ip][ir]+dzv[nz-4][ip][ir])/(0.5*zhval[nz-1]) + val[nz-2][ip][ir];
                    break;
                case IS_TEMPERATURE:
                case IS_TE:
                    FORYX_BD
                    {
                        // kappa_t * dz phi = dz T a Neumann condition
                        tval = 0.;
...
void Cyto_CalcParameters(PARA *p,HDF_DS *d)
{
    ...
    p->nprof = p->part_source/p->n0*p->unit_time*1e-6;
    p->tprof = p->temp_source/p->n0/p->Te*p->unit_time*1e-6;
