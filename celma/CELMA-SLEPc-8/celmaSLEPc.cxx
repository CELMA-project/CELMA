...
#include "celmaSLEPc.hxx"

// Initialization of the physics
// ############################################################################
int CelmaSLEPc::init(bool restarting) {
    TRACE("Halt in CelmaSLEPc::init");
    ...
    // ************************************************************************
    // The readFrom section sets the option for the file reader
    fieldReader = Datafile(Options::getRoot()->getSection("readFrom"));

    /* Add fields to be read
     * First keyword denotes where the field is going to be saved
    ...
    // ........................................................................
    options = Options::getRoot()->getSection("readFrom");

    if(options->isSet("dir")) {
      // Input specific read from directory
      options->get("dir", readFromDir, "data");
    }
    else {
      // Use the root data directory
      Options::getRoot()->get("datadir", readFromDir, "data");
    }

    options->get("extension", readFromExtension, "nc"          );
    options->get("name",      readFromFileName,  "BOUT.restart");
    // FromFileName will be placed into an vsprintf function, and needs the
    // following substring in from and back of the filename
    output << "Reading " << readFromFileName << "." << BoutComm::rank() << "."
           << readFromExtension << " from " << readFromDir << std::endl;
    readFromFileName.insert(0,"%s/");
    readFromFileName.append(".%s");
    // ........................................................................

    /* Open the file
     * First argument is the name to open
     * Second is the directory
     * Third is the extention
     */
    if(!fieldReader.openr(
                          readFromFileName.c_str(),
                          readFromDir.c_str(),
                          readFromExtension.c_str()
                          )){
      throw BoutException("Error: Could not open readFromFile\n");
    }
    // Read the specified variables
    if(!fieldReader.read()){
      throw BoutException("Error: Could not read readFromFile\n");
    }
    fieldReader.close();
    // ************************************************************************
...
// ############################################################################
int CelmaSLEPc::rhs(BoutReal t){
    TRACE("Halt in CelmaSLEPc::rhs");
