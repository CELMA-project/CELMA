...
int Celma::rhs(BoutReal t) {
  ...
  ddt(vortD) = vortNeutral + potNeutral + parDerDivUIParNGradPerpPhi +
               divParCur + vortDAdv + kinEnAdvN + vortDParArtVisc +
               vortDPerpArtVisc + vortDHyperVisc;
...
void Celma::setAndSaveViscosities() {
  ...
  if (!constViscPar) {
    ...
    artViscParVortD *= SQ(coord->dy(0, 0));
  }
  ...
  if (!constViscPerp) {
    ...
    artViscPerpVortD *= SQ(coord->dx(0, 0));
  }
  ...
  if (!constViscHyper) {
    ...
    artHyperAzVortD *= SQ(SQ(coord->dz));
  }
