...
int Celma::init(bool restarting) {
    ...
    // Determine spectral filtering
    // ************************************************************************
    // 2/3 of number of gridpoints
    // NOTE: Currently GlobalNz returns MZ, which has the additional z
    //       plane
    maxMode = int(floor(2.0*(mesh->GlobalNz - 1)/3.0));
    // ************************************************************************
...
int Celma::rhs(BoutReal t) {
    ...
    ddt(lnN) = lowPass(ddt(lnN), maxMode);
    // ************************************************************************
    ...
    ddt(uEPar) =
        ...
        ;
    // Filtering highest modes
    ddt(uEPar) = lowPass(ddt(uEPar), maxMode);
    // ************************************************************************
    ...
    ddt(uIPar) =
        ...
        ;
    // Filtering highest modes
    ddt(uIPar) = lowPass(ddt(uIPar), maxMode);
    // ************************************************************************
    ...
    ddt(vortD) =
            ...
            ;
    // Filtering highest modes
    ddt(vortD) = lowPass(ddt(vortD), maxMode);
    // ************************************************************************
    return 0;
}
// ############################################################################
