...
int CelmaCurMom::init(bool restarting)
{
    ...
    if(saveTerms){
        ...
        SAVE_REPEAT (jParVisc);
        SAVE_REPEAT2(jParParArtVisc, jParPerpArtVisc);
        // momDensPar terms
        SAVE_REPEAT2(momDensAdv, elPressure);
        SAVE_REPEAT2(densDiffusion, momDensVisc);
        SAVE_REPEAT2(momDensParArtVisc, momDensPerpArtVisc);
        // Vorticity terms
...
int CelmaCurMom::rhs(BoutReal t)
{
    ...
    jParVisc        = - mu*(4.0*eta0ENorm/3.0)*D2DY2(uEPar);
 ...
 // neutralIRes calculated above
    ...
    momDensVisc        = (4.0/3.0)*(eta0INorm*D2DY2(uIPar)
                                    + eta0ENorm*D2DY2(uEPar));
...
void CelmaCurMom::setAndSaveParameters()
{
    ...
    SAVE_ONCE2(eta0INorm, eta0ENorm);
    // ************************************************************************
}
...
void CelmaCurMom::setAndSaveViscosities()
{
    ...
    // Calculate diffusion from grid size, print and store the variables
    ...
    for (std::vector<BoutReal>::iterator it = perpVisc.begin();
         it != perpVisc.end();
         ++it){
        if(*it > 10.0*eta0INorm){
            throw BoutException("One of the parallel viscosities "
                                "is 10.0 times larger than eta0I");
        }
    }
