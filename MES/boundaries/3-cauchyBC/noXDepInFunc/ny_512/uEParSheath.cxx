...
int UeSheath::init(bool restarting) {
    ...
    // ************************************************************************
    // uEParOrigin
    uEParOrigin = FieldFactory::get()
    ->create3D("uEPar:function", Options::getRoot(), mesh, CELL_CENTRE, 0);
    // phi
    phi = FieldFactory::get()
    ->create3D("phi:function", Options::getRoot(), mesh, CELL_CENTRE, 0);
    // profile
    profile = FieldFactory::get()
    ...
    // Extrapolate
    ownBC.cauchyBC(uEParWBC, phi, Lambda, phiRef, profile);
